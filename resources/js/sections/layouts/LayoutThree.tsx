...
const backgroundBrandColors = [{ preset: 'preset-brand-1' }, { preset: 'preset-brand-2' }, { preset: 'preset-brand-3' }];
...
export default function LayoutThree() {
  const handleColorChange = (preset: SetStateAction<string>) => {
    document.body.className = `layout-3 ${preset}`;
  };
  ...
  return (
    <>
      ...
      <MainCard>
        ...
        <small>
          set background class in <code>.preset</code> and <code>.preset-*</code> in <code>&lt;body&gt;</code>
        </small>
      ...
      <MainCard>
        ...
        <small>
          set background class in <code>.preset</code> and <code>.preset-gradient-*</code> in <code>&lt;body&gt;</code>
        </small>

        <Row className="g-3 mt-3">
          {backgroundGradients.map((gradient, index) => (
            <Col xs="auto" key={index}>
              <a onClick={() => handleGradientChange(gradient.preset)} className={`hei-40 wid-40 d-inline-block preset ${gradient.preset}`}>
                <i className="ti ti-brush text-white f-18 d-flex align-items-center hei-40 justify-content-center" />
              </a>
            </Col>
          ))}
